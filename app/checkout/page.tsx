...
export default async function CheckoutPage({ searchParams }: { searchParams: SearchParams }) {
  const { product: slug, start, end } = searchParams;
  ...
  const { checkDateRangeAvailability } = await import("@/lib/availability");
  ...
  const available = await checkDateRangeAvailability(product!.id, start!, end!);
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center py-16 px-4">
        <div className="max-w-md text-center bg-white rounded-2xl shadow-lg p-8">
          ...
          <p className="text-gray-600 mb-6">
            {product!.name} is no longer available for those dates. Let&apos;s find you something else.
          </p>
